...
ComputeStyle(activepressure,ComputeActivePressure)

#else

#ifndef LMP_COMPUTE_ACTIVEPRESSURE_H
#define LMP_COMPUTE_ACTIVEPRESSURE_H

#include "compute.h"

namespace LAMMPS_NS {

class ComputeActivePressure : public Compute {
 public:
  ComputeActivePressure(class LAMMPS *, int, char **);
  virtual ~ComputeActivePressure();
  virtual void init();
 ...
 protected:
  ...
  int unwrapcoordsflag; // 1 if non-periodic coordinates are used in pressure
  int fixflag; need to check if this is okay to keep.
