...
#include "compute_threebodyangleintegral.h"
#include <mpi.h>
#include <stdio.h>
#include <cmath>
#include <cstring>
...
ComputeThreeBodyAngleIntegral::ComputeThreeBodyAngleIntegral(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  rdfpair(NULL), ilo(NULL), ihi(NULL), jlo(NULL), jhi(NULL),
  hist(NULL), histall(NULL), typecount(NULL), icount(NULL), jcount(NULL),
  duplicates(NULL)
{
  if (narg != 5 && narg != 7 && narg != 9)
    error->all(FLERR,"Illegal compute three body angle integral command");

  array_flag = 1;
  extarray = 0;

  nbin_dist = force->inumeric(FLERR,arg[3]);
  if (nbin_dist < 1) error->all(FLERR,"Illegal compute three body "
				"angle integral command");

  nbin_theta = force->inumeric(FLERR,arg[4]);
  if (nbin_theta < 1) error->all(FLERR,"Illegal compute three body "
				"angle integral command");

  
  nbin_total = nbin_dist*nbin_dist;
  // optional args
  ...
  if (narg > 5) {
	...
	} else error->all(FLERR,"Illegal compute threebody angle "
			  "integral command");
      }
  ...
  printf("lower_cut = %f, cutoff_user = %f\n",
	 lower_cut,cutoff_user);
  if (force->newton_pair) {
    error->all(FLERR,"compute threebody command angle integral "
	       "is incompatible with newton pair being on.");
  }
  ...
  if (ntypes != 1) {
    error->all(FLERR,"Cannot compute threebody angle integral for "
	       "system with multiple atom types.");
  }
...
ComputeThreeBodyAngleIntegral::~ComputeThreeBodyAngleIntegral()
{
...
void ComputeThreeBodyAngleIntegral::init()
{
  ...
  if (cutflag) {
    ...
    delr = (cutoff_user - lower_cut) / nbin_dist;
  } else delr = (force->pair->cutforce - lower_cut) / nbin_dist;
...
void ComputeThreeBodyAngleIntegral::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void ComputeThreeBodyAngleIntegral::init_norm()
{
  int i,j,m;
...
void ComputeThreeBodyAngleIntegral::compute_array()
// ======================================================================
...
// ======================================================================
{
  ...
  for (i = 0; i < nbin_dist; i++)
    for (j = 0; j < nbin_dist; j++)
      for (k = 0; k < nbin_theta; k++)
	hist[i][j][k] = 0;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	ij_bin = static_cast<int> ((rij-lower_cut)*delrinv);
	ik_bin = static_cast<int> ((rik-lower_cut)*delrinv);
	dum_jk_bin = static_cast<int> ((rjk-lower_cut)*delrinv);
	...
	hist[ij_bin][ik_bin][theta_bin] += 1.0;
...
void ComputeThreeBodyAngleIntegral::set_array(double constant, double normfac)
{
  ...
  for (ij_bin = 0; ij_bin < nbin_dist; ij_bin++) {
    ulower = ij_bin*delr;
    uupper = (ij_bin+1)*delr;
    for (ik_bin = 0; ik_bin < nbin_dist; ik_bin++) {
      vlower = ik_bin*delr;
      vupper = (ik_bin+1)*delr;	  
	...
	// sloppy integration.. but oh well.
	
	costheta = cos((theta_bin+0.5)*deltheta);
	
	if (vfrac * normfac != 0.0) {
	  gr += costheta*histall[ij_bin][ik_bin][theta_bin]/(vfrac *normfac);
	} else {
	  gr += 0.0;
	}
      }
      flat_index = ik_bin+ ij_bin * nbin_dist;
      array[flat_index][0] = (ij_bin + 0.5)*delr+lower_cut;
...
double ComputeThreeBodyAngleIntegral::compute_rjk(double rij, double rik, double theta)
{
